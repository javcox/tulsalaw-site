...
function getTimingScore(bucket: string, flags: string[]) {
	switch (bucket) {
		...
		case 'more-than-2-years':
			...
			return -2;
		case 'exact-date-known': return 2;
		default: return 0;
	}
}
...
function getPiLiabilityScore(payload: IntakeSubmission, flags: string[]) {
	switch (asString(payload.piLiabilityAssessment)) {
		case 'clear-other-party': return 5;
		case 'likely-other-party': return 3;
...
function getPiDamagesScore(payload: IntakeSubmission, flags: string[]) {
	switch (asString(payload.piTreatmentStatus)) {
		case 'er-ongoing-major': return 5;
		case 'ongoing-treatment': return 4;
		case 'initial-treatment-only': return 2;
		case 'minor-or-vague':
...
function getPiInsuranceScore(payload: IntakeSubmission) {
	switch (asString(payload.piInsuranceStatus)) {
		...
		case 'commercial-insurance': return 2;
		case 'likely-insurance': return 1;
		case 'no-insurance-known': return -1;
		default: return 0;
	}
}
...
function evaluatePersonalInjury(payload: IntakeSubmission) {
	...
	if (assessment === 'clear-other-party' && treatment === 'er-ongoing-major') score = Math.max(score, 14);
...
function evaluateGeneral(payload: IntakeSubmission) {
	...
	if (service === 'uncontested-divorce') {
		if (yesNo(payload.divorceBothAgree) === 'yes' && yesNo(payload.divorcePropertyAgree) === 'yes') score += 3;
		else {
...
function buildTextSummary(payload: IntakeSubmission, evaluation: { score: number; flags: string[]; route: IntakeRoute }, serviceLabel: string) {
	...
	if (serviceLabel === 'Personal Injury') {
		...
		getGeneralServiceRows(asString(payload.service), payload).forEach(([label, value]) => pushIfValue(lines, label, value));
	}
...
function buildHtmlSummary(payload: IntakeSubmission, evaluation: { score: number; flags: string[]; route: IntakeRoute }, serviceLabel: string) {
	...
	sections.push(buildHtmlSection('Top Summary', [
	...
	if (serviceLabel === 'Personal Injury') {
		sections.push(buildHtmlSection('Personal Injury Snapshot', [
		...
		sections.push(buildHtmlSection('Liability', [
		...
		sections.push(buildHtmlSection('Damages', [
		...
		sections.push(buildHtmlSection('Matter', [
	...
	sections.push(buildHtmlSection('Special Notes', [
	...
	sections.push(buildHtmlSection(
		'Flags',
