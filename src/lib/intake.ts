export type IntakeRoute = 'green' | 'yellow' | 'red' | 'black';
...
export const intakeServices: IntakeOption[] = [
...
export type IntakeSubmission = Record<string, string | string[] | boolean | number | null | undefined>;
...
const familyServices = new Set(['family-law', 'uncontested-divorce', 'guardianship']);
...
function buildHtmlSection(title: string, rows: Array<[string, unknown]>) {
	const items = rows
		.map(([label, value]) => [label, asString(value)] as const)
		.filter(([, value]) => value)
...
function getGeneralServiceRows(service: string, payload: IntakeSubmission): Array<[string, unknown]> {
...
function getTimingScore(bucket: string, flags: string[]) {
	switch (bucket) {
		case 'within-30-days':
			return 3;
		case '1-6-months':
...
function getPiLiabilityScore(payload: IntakeSubmission, flags: string[]) {
...
function getPiDamagesScore(payload: IntakeSubmission, flags: string[]) {
...
function getPiEconomicScore(payload: IntakeSubmission, flags: string[]) {
	...
	if (bills === '10k-plus' || missedWork === 'major' || propertyDamage === 'major') {
		return 3;
	}

	if (bills === '1k-10k' || missedWork === 'moderate' || propertyDamage === 'moderate') {
		return 2;
	}
...
function getPiInsuranceScore(payload: IntakeSubmission) {
	const insurance = asString(payload.piInsuranceStatus);
	switch (insurance) {
...
function evaluatePersonalInjury(payload: IntakeSubmission) {
	const flags: string[] = [];
	let score = 0;
	...
	const workingAtTime = yesNo(payload.piWorkingAtTime) === 'yes';
	if (workingAtTime) {
		flags.push('Possible workers comp crossover');
	}

	if (yesNo(payload.piRecoveryLog) === 'yes') {
		score += 1;
	}
	...
	if (subtype === 'rear-end' && treatment !== 'no-treatment') {
		score = Math.max(score, 13);
	}
	if (subtype === 'commercial-vehicle' || subtype === 'trucking') {
		score = Math.max(score, 13);
	}
	...
	let route: IntakeRoute = 'yellow';
	if (
		flags.includes('Self fault') ||
		flags.includes('No treatment') ||
		score <= 6
	) {
		route = 'red';
	} else if (score >= 13) {
...
function evaluateGeneral(payload: IntakeSubmission) {
	const flags: string[] = [];
	...
	if (!intakeServiceLabelMap[service]) {
		...
		return { score: 0, flags, route: 'black' as IntakeRoute };
	}
	...
	if (yesNo(payload.consultationOkay) === 'yes') {
		score += 5;
	...
	if (asString(payload.county)) {
		score += 1;
	}

	if (yesNo(payload.decisionMaker) === 'yes') {
		score += 2;
	}

	if (planningServices.has(service)) {
		if (yesNo(payload.planSignerAvailable) === 'yes') {
			score += 2;
		} else {
		...
		if (asString(payload.planDepth) === 'customized') {
			score += 1;
		}
	}
	...
	if (service === 'guardianship') {
		...
		if (yesNo(payload.guardianshipAgreement) === 'yes') {
			score += 2;
		}
	}
	...
	if (service === 'legal-guidance') {
		if (yesNo(payload.guidanceAlreadyFiled) === 'yes') {
			score += 1;
		}
	...
	if (businessServices.has(service)) {
		...
		if (asString(payload.businessIntent) === 'prevent') {
			score += 3;
		}
		if (asString(payload.businessIntent) === 'cleanup') {
			score += 1;
		}
	}
	...
	let route: IntakeRoute = 'yellow';
	if (flags.includes('Outside Oklahoma')) {
		route = 'black';
	} else if (score >= 14) {
		route = 'green';
	} else if (score >= 8) {
...
function detectSpam(payload: IntakeSubmission) {
	const combined = [
...
function buildTextSummary(payload: IntakeSubmission, evaluation: { score: number; flags: string[]; route: IntakeRoute }, serviceLabel: string) {
	const lines: string[] = [];
	lines.push(`SERVICE`);
	...
	if (serviceLabel === 'Personal Injury') {
		...
		pushIfValue(lines, 'Who is responsible and why', `${asString(payload.piResponsibleParty)} | ${asString(payload.piResponsibilityWhy)}`);
	...
	if (evaluation.flags.length) {
		evaluation.flags.forEach((flag) => lines.push(`- ${flag}`));
	} else {
...
function buildHtmlSummary(payload: IntakeSubmission, evaluation: { score: number; flags: string[]; route: IntakeRoute }, serviceLabel: string) {
	...
	sections.push(
		buildHtmlSection(
			'Flags',
			evaluation.flags.length ? evaluation.flags.map((flag) => [flag, 'Triggered']) : [['Flags', 'None triggered']],
		),
	);
...
export function evaluateIntake(payload: IntakeSubmission): IntakeEvaluation {
	const service = asString(payload.service);
	...
	if (startedAt && Date.now() - startedAt < 3500) {
		return {
			...
			executiveSummary: buildTextSummary(payload, { route: 'black', score: 0, flags: ['Submitted too quickly'] }, serviceLabel),
			htmlSummary: buildHtmlSummary(payload, { route: 'black', score: 0, flags: ['Submitted too quickly'] }, serviceLabel),
		};
	}
	...
	let result =
		service === 'personal-injury' ? evaluatePersonalInjury(payload) : evaluateGeneral(payload);

	if (yesNo(payload.inOklahoma) !== 'yes') {
		result = {
			score: 0,
			flags: [...result.flags, 'Outside Oklahoma'],
			route: 'black',
		};
	}
